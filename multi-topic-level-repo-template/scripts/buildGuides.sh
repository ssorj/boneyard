...
BOOKS_SRC="$( dirname $SCRIPT_SRC )/books"
...
EXAMPLES:
  ...
  Build a specific guide(s) from $BOOKS_SRC:
    $0 My_Title_A
    $0 My_Title_A
    $0 My_Title_C
EOM
...
# Use $BOOKS_SRC so we don't have to worry about relative paths.
cd $BOOKS_SRC
...
do
  echo ""
  echo "Building $BOOKS_SRC/${subdir##*/}"
  # Navigate to the dirctory and build it
  if ! [ -e $BOOKS_SRC/${subdir##*/} ]; then
    BUILD_MESSAGE="$BUILD_MESSAGE\nERROR: $BOOKS_SRC/${subdir##*/} does not exist."
    continue
  fi
  cd $BOOKS_SRC/${subdir##*/}
  GUIDE_NAME=${PWD##*/}
