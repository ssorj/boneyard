...
log = logging.getLogger("ptolemy.summary")
...
class CycleSummary(object):
    ...
    def get_title(self):
        server = self.cycle.project.server.name
    ...
    def generate_properties(self, out):
        ...
        out.append("  %-10s  %s" % ("Server", self.cycle.project.server.name))
        ...
        out.append("  %-10s  %s" % ("URL", self.cycle.url))
    ...
    def generate_output(self, out, script_name):
        ...
        out.append("Output")
        out.append("")
        out.append("  [%s/%s]" % (self.cycle.url, output))
        out.append("")
