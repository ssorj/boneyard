...
_log = logger("turret.parameter")

class Parameter:
    """
    Capture and validate named state from the request

    Parameter instances are children of L{Procedure}.  Parameter values are
    stored on the current L{Request}.
    ...
    @ivar procedure: The parent procedure
    ...
    @group Procedure attributes: get*, set*, add_error
    ...
    def __init__(self, procedure, name):
        ...
        assert self.name not in self.procedure.parameters_by_name

        self.procedure.parameters.append(self)
        self.procedure.parameters_by_name[self.name] = self
