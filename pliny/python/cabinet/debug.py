...
def get_parameters(session):
    return session._parameter_values_by_parameter

def get_session_info(session):
    return session._attributes_by_name
...
def get_client_info(session):
    client = session.page.get_client_session(session)

    if client is None:
        return

    return client._attributes_by_name
...
class _BenchmarkHarness(object):
