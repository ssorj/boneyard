...
class ThreadPool(object):
    ...
    def start(self):
        ...
        for worker in self._threads:
            # XXX - JR - Modified for nicer logging output
            #worker.setName("CP Server " + worker.getName())
            worker.setName("web-%02i" % self._threads.index(worker))
            # End modification
            worker.start()
        for worker in self._threads:
    ...
    def _spawn_worker(self):
        ...
        # XXX - JR - Modified for nicer logging output
        #worker.setName("CP Server " + worker.getName())
        worker.setName("web-%02i" % len(self._threads))
        # End modification
        worker.start()
        return worker
        ...
        try:
            ...
            if self.ssl_adapter is not None:
                try:
                    ...
                    # XXX - JR - Modified to redirect to SSL
                    ...
                    # XXX I don't have path info at this point
                    location = "https://%s:%i/" % self.bind_addr
                    buf = ["%s 307 Temporary Redirect\r\n" % self.protocol,
                           "Location: %s\r\n" % location,
                           "Content-Length: 0\r\n\r\n",]
